...
Once tokens are obtained, they are stripped of leading and trailing punctuation (\textbox{":;,.-"})% (\textbox{``:", ``;", ``,", ``.", ``-"})%(\textbox{":;,.- "})
.
After that, tokens corresponding to common English stopwords\footnote{Refer to \texttt{pyspark.ml.feature.StopWordsRemover} documentation for a full list} or unuseful punctuation (\textbox{":-+"}) %(\textbox{[``:", ``-", ``+"]}) 
are discarded.

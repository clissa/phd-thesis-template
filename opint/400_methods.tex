...
This section presents a detailed description of an approach for dealing with FTS transfer failures. % \cref{sec:FTS_failures}.
...
To achieve that, the \textit{Spark} \cite{zaharia2010spark} processing framework is used through the \texttt{pyspark} language to leverage the advantages of distributed computations for large data. \sidenote[Luca][notesyellow]{Aggiungere reference a implementazione tramite OpInt framework? Ci starebbe ma forse richiederebbe la descrizione che richiede tanto tempo e non è fondamentale per il resto del discorso}
...
The clustering stage, instead, is performed online every day so to always expose the latest results to the on-duty shifters (see %Sections

...
In particular, preliminary results have shown that changing from the \texttt{keras} initialization of the batch normalization layers -- \textbox{momentum=0.01, eps=0.001} -- to that adopted by \texttt{fastai} -- \textbox{momentum=0.1, eps=1e-5} -- seem to produce predictions with sharper boundaries (cf. top and bottom rows in \cref{fig:kerasVSfastai_bathnorm}).
\begin{figure}
\centering
...
\sidenote[Luca][notesyellow]{spiegare meglio il riferimento alla doppia marcatura}
